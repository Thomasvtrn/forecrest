...
var PAGES = [
  ...
  { group: { fr: "Finance", en: "Finance" }, items: [
    { tab: "streams", label: "Revenus", file: "finance/RevenueStreamsPage.jsx", desc: { fr: "Sources de revenus avec 9 comportements (recurring, project, hourly...). DataTable + modal split-panel. Donut par catégorie.", en: "Revenue sources with 9 behaviors. DataTable + split-panel modal. Category donut." }, props: "streams, setStreams, annC, businessType, chartPalette" },
    { tab: "opex", label: "Charges", file: "finance/OperatingCostsPage.jsx", desc: { fr: "Charges d'exploitation avec 10 catégories PCMN. Auto-items (amortissements, intérêts, avantages, crowdfunding). TVA par ligne.", en: "Operating costs with 10 PCMN categories. Auto-items. Per-line VAT." }, props: "costs, setCosts, cfg, totalRevenue, debts, assets, sals, crowdfunding, chartPalette" },
    { tab: "income_statement", label: "Compte de résultat", file: "finance/IncomeStatementPage.jsx", desc: { fr: "P&L complet avec cascade PCMN.", en: "Full P&L with PCMN cascade." }, props: "streams, costs, cfg, assets, stocks, salCosts, annualInterest, chartPalette" },
    { tab: "balance", label: "Bilan", file: "finance/BalanceSheetPage.jsx", desc: { fr: "Bilan prévisionnel actif/passif.", en: "Projected balance sheet assets/liabilities." }, props: "cfg, streams, costs, sals, debts, assets, stocks, crowdfunding, totalRevenue, chartPalette" },
    { tab: "cashflow", label: "Trésorerie", file: "finance/CashFlowPage.jsx", desc: { fr: "Projection de trésorerie multi-années. Insight cards (cash, runway, croissance).", en: "Multi-year cash projection. Insight cards (cash, runway, growth)." }, props: "totalRevenue, monthlyCosts, annC, ebitda, cfg, setCfg" },
    { tab: "accounting", label: "Comptabilité", file: "finance/AccountingPage.jsx", desc: { fr: "Plan comptable PCMN pliable par classe, export CSV/belge.", en: "PCMN chart of accounts collapsible by class, CSV/Belgian export." }, props: "costs, sals, cfg, debts, streams, stocks, totalRevenue, monthlyCosts" },
    { tab: "ratios", label: "Santé financière", file: "finance/RatiosPage.jsx", desc: { fr: "7 sections : rentabilité, liquidité, dette, ROI, runway, opérationnel, BFR. KPI cards, FinanceLinks, gauges, explainers.", en: "7 sections: profitability, liquidity, debt, ROI, runway, operational, WCR. KPI cards, FinanceLinks, gauges, explainers." }, props: "cfg, totalRevenue, monthlyCosts, ebit, netP, resLeg, debts, sals, salCosts, stocks" },
    { tab: "debt", label: "Financement", file: "finance/DebtPage.jsx", desc: { fr: "7 types de dette (banque, crédit, leasing, prêt, subside, avance, convertible). Échéancier.", en: "7 debt types. Repayment schedule." }, props: "debts, setDebts, ebitda, capitalSocial, crowdfunding, chartPalette" },
    { tab: "crowdfunding", label: "Crowdfunding", file: "finance/CrowdfundingPage.jsx", desc: { fr: "Wizard d'activation (4 étapes). 6 plateformes. Gestion des paliers.", en: "Activation wizard (4 steps). 6 platforms. Tier management." }, props: "crowdfunding, setCrowdfunding, chartPalette" },
    { tab: "equity", label: "Intéressement (ESOP)", file: "finance/EquityPage.jsx", desc: { fr: "Bons de souscription et options sur actions. Auto-sync ESOP vers cap table.", en: "Subscription warrants and stock options. Auto-sync ESOP to cap table." }, props: "grants, setGrants, poolSize, setPoolSize, esopEnabled, setEsopEnabled, sals, shareholders" },
    { tab: "captable", label: "Cap Table", file: "finance/CapTablePage.jsx", desc: { fr: "Table de capitalisation, modale split-panel, donut de répartition, simulateur de levée.", en: "Cap table, split-panel modal, distribution donut, round simulator." }, props: "shareholders, setShareholders, roundSim, setRoundSim, grants, sals, cfg, chartPalette" },
    { tab: "pact", label: "Pacte", file: "finance/PactPage.jsx", desc: { fr: "Clauses du pacte d'actionnaires configurables.", en: "Configurable shareholder agreement clauses." }, props: "cfg, setCfg" },
  ]},
  { group: { fr: "Entreprise", en: "Company" }, items: [
    { tab: "salaries", label: "Rémunérations", file: "company/SalaryPage.jsx", desc: { fr: "6 types de contrat. ATN avec leasing/achat. Auto-création dans équipements.", en: "6 contract types. Benefits with lease/purchase. Auto-creation in equipment." }, props: "sals, setSals, cfg, salCosts, arrV, assets, setAssets, chartPalette" },
    { tab: "equipment", label: "Équipements", file: "company/AmortissementPage.jsx", desc: { fr: "Immobilisations (classes 2x PCMN) avec amortissement linéaire/dégressif.", en: "Fixed assets (PCMN class 2x) with linear/declining depreciation." }, props: "assets, setAssets, cfg, chartPalette" },
    { tab: "stocks", label: "Stocks", file: "company/StocksPage.jsx", desc: { fr: "Gestion des stocks avec méthodes de valorisation.", en: "Stock management with valuation methods." }, props: "stocks, setStocks, cfg, chartPalette" },
    { tab: "production", label: "Rentabilité", file: "company/ProductionPage.jsx", desc: { fr: "Calcul de rentabilité et gestion de recettes/production.", en: "Profitability calculation and recipe/production management." }, props: "production, setProduction, streams, chartPalette" },
    { tab: "profile", label: "Profil", file: "company/ProfilePage.jsx", desc: { fr: "Informations entreprise, responsable légal, couleur d'accent.", en: "Company info, legal representative, accent color." }, props: "cfg, setCfg" },
  ]},
  { group: { fr: "Marketing", en: "Marketing" }, items: [
    { tab: "marketing", label: "Vue d'ensemble", file: "marketing/MarketingPage.jsx", desc: { fr: "Dashboard marketing : KPIs, entonnoir de conversion, métriques clés expliquées, navigation rapide.", en: "Marketing dashboard: KPIs, conversion funnel, key metrics explained, quick navigation." }, props: "marketing, setMarketing, cfg, activeTab, setTab, chartPalette" },
    { tab: "mkt_channels", label: "Canaux", file: "marketing/MarketingPage.jsx", desc: { fr: "Gestion des canaux d'acquisition (Meta, Google, LinkedIn, TikTok, SEO, E-mailing). DataTable + modal split-panel.", en: "Acquisition channel management. DataTable + split-panel modal." }, props: "" },
    { tab: "mkt_campaigns", label: "Campagnes", file: "marketing/MarketingPage.jsx", desc: { fr: "Création et suivi de campagnes par canal, objectif et statut. Sub-tabs par statut.", en: "Campaign creation and tracking by channel, objective and status. Status sub-tabs." }, props: "" },
    { tab: "mkt_budget", label: "Budget", file: "marketing/MarketingPage.jsx", desc: { fr: "Répartition du budget par canal avec donut interactif.", en: "Budget breakdown by channel with interactive donut." }, props: "" },
    { tab: "mkt_conversions", label: "Conversions", file: "marketing/MarketingPage.jsx", desc: { fr: "Entonnoir d'acquisition et comparaison des canaux par ROAS.", en: "Acquisition funnel and channel comparison by ROAS." }, props: "" },
  ]},
  { group: { fr: "Analyse", en: "Analysis" }, items: [
    { tab: "sensitivity", label: "Sensibilité", file: "analysis/SensitivityPage.jsx", desc: { fr: "Tornado chart : impact de chaque paramètre sur l'EBITDA. Analyse what-if.", en: "Tornado chart: impact of each parameter on EBITDA. What-if analysis." }, props: "totalRevenue, monthlyCosts, salCosts, ebitda, cfg" },
  ]},
  { group: { fr: "Boîte à outils", en: "Toolkit" }, items: [
    { tab: "tool_qr", label: "QR Code", file: "tools/ToolsPage.jsx", desc: { fr: "Générateur de QR code : 9 types. Thèmes de couleurs, logo au centre, historique persisté.", en: "QR code generator: 9 types. Color themes, center logo, persisted history." }, props: "activeTab" },
    { tab: "tool_domain", label: "Domaines", file: "tools/ToolsPage.jsx (DomainChecker)", desc: { fr: "Vérificateur de nom de domaine : 15 TLDs, score de qualité, suggestions, watchlist.", en: "Domain name checker: 15 TLDs, quality score, suggestions, watchlist." }, props: "activeTab" },
  ]},
  { group: { fr: "Système", en: "System" }, items: [
    { tab: "set", label: "Paramètres", file: "meta/SettingsPage.jsx", desc: { fr: "Fiscalité, comptabilité, apparence, alertes, raccourcis.", en: "Fiscal, accounting, appearance, alerts, shortcuts." }, props: "cfg, setCfg" },
    { tab: "changelog", label: "Notes de version", file: "meta/ChangelogPage.jsx", desc: { fr: "Changelog auto-généré depuis changelog.js + i18n.", en: "Auto-generated changelog from changelog.js + i18n." }, props: "" },
    { tab: "credits", label: "Crédits", file: "meta/CreditsPage.jsx", desc: { fr: "Licences open-source de toutes les dépendances.", en: "Open-source licenses of all dependencies." }, props: "" },
  ]},
...
var COMPONENTS = [
  ...
  { name: "ButtonGroup", file: "ButtonGroup.jsx", desc: "Connected button sets with basic, radio, checkbox and disabled patterns" },
  ...
  { name: "SearchInput", file: "SearchField.jsx", desc: "Search field with inline icon and clear control" },
